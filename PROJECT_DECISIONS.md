...
## DEC-040 — Sourced N4/N3 content uses pinned provenance, lower-level prompt identity and source-language fallback
Status: ACCEPTED
Date: 2026-09-24
Decision: N4/N3 Vocabulary and Kanji may become active only from a pinned, redistributable source snapshot with preserved attribution and reproducible transformation. For the OpenJLPT slice, ZaPan uses lower-level-prompt-wins identity (existing N5 > N4 > N3): if a Vocabulary term or Kanji character already exists at a lower level, the higher-level record is excluded from learner SRS identity but retained in the raw source snapshot/exclusion report. Imported cards may display source-backed English when reviewed Vietnamese is absent; AI-generated bulk translation is not treated as sourced content.
Reason: ZaPan currently quizzes Vocabulary by term and Kanji by character, so duplicate prompts across community level lists would create parallel SRS records for the same learner prompt. OpenJLPT itself states that current JLPT Vocabulary/Kanji lists are unofficial approximations, and its dataset carries attribution/share-alike obligations. Inventing Vietnamese glosses or treating community groupings as official would violate the source-grounded content rule.
Consequence: source commit, hashes, raw counts, retained counts and every excluded prompt remain inspectable. N4/N3 UI labels them open study sets rather than official JLPT lists. Future source refreshes must regenerate and rerun count/dedupe/content/browser gates before replacement.
