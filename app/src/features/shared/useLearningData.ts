...
async function loadSnapshot(content: ContentRepository, learning: LearningRepository): Promise<LearningDataState> {
  ...
  return { loading: false, error: null, cards, progress, overview: deriveLearningOverview(cards, progress, capturedAt), capturedAt }
}

export function useLearningData(): LearningDataState & { refresh: () => Promise<void> } {
  const { content, learning } = useAppServices()
  const [state, setState] = useState<LearningDataState>({ loading: true, error: null, cards: [], progress: [], overview: EMPTY_OVERVIEW, capturedAt: 0 })
